...
exports.handler = function(input, context) {
	...
	zlib.gunzip(zippedInput, async function(error, buffer) {
		...
		const elasticsearchBulkData = await transform(awslogsData);

		// skip control messages
		if (!elasticsearchBulkData) {
			console.info('Received a control message');
...
async function transform(payload) {
	...
	await Promise.all(
		payload.logEvents.map(async logEvent => {
			...
			const indexName =
				...
				'-' +
				payload.logGroup.replace(/\//g, '').toLowerCase();
